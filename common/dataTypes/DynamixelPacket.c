...
uint8_t   DynamixelPacketGetPayloadSize(DynamixelPacket * packet)
{
  return packet->buffer[DYNAMIXEL_PACKET_POS_SIZE] - 2;  //subtract the instruction and checksum
}

uint8_t   DynamixelPacketRawGetPayloadSize(uint8_t * packet)
{
  return packet[DYNAMIXEL_PACKET_POS_SIZE] -2;
}
