...
void writeBMP(const unsigned char cover_map[], const unsigned char cost_map[], const int dijkstra[], const int mapm, const int mapn, const std::vector<Traj_pt_s> & traj, const char * filename) {
	BMPType bt;
	BMPFileHeader bfh;
	BMPInfoHeader bih;
	ofstream fout;
	...
	for(int j =0; j< mapn; j++) {
		for(int i=0; i< mapm; i++) {
			...
			fout.put((unsigned char)((dijkstra[i+j*mapm]/100) % 250)); //red
	...
	//for (int idx =0; idx < traj.size(); idx++) {
		//streampos pos = (traj[idx].x + traj[idx].y*sizex)*3 + sizeof(BMPType) + sizeof(BMPFileHeader) + sizeof(BMPInfoHeader);
