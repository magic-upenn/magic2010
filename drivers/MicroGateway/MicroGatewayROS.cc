...
MicroGateway::MicroGateway()
{
  ...
  this->last_time = ros::Time::now(); //set up the first time sample 
...
/////////////////////////////////////////////////////////////////////////
// Connect to (and handle) ROS stuff (including odometry calculations)
/////////////////////////////////////////////////////////////////////////

void MicroGateway::cmdVelCallback(const geometry_msgs::Twist::ConstPtr& cmd_msg) { //double x, double rot) { 
	//this->lock.lock(); 
	...
	this->wheelSpeed[LEFT] = this->x_ + this->rot_ * this->wheelSeparation / 2.0; 
	this->wheelSpeed[RIGHT] = this->x_ - this->rot_ * this->wheelSeparation / 2.0; 

	VelocityCmd vcmd; 
	vcmd.vCmd = this->x_ * 127; //this->wheelSpeed[LEFT]; 
	vcmd.wCmd = this->rot_ * 127; //this->wheelSpeed[RIGHT]; 
...
void internalCmdVelCallback(const geometry_msgs::Twist::ConstPtr& cmd_msg) { 
	...
	MicroGateway::cmdVelCallback(cmd_msg->linear.x, cmd_msg->angular.z); 

}
*/
//much of the ROS code is based on or taken directly from the Gazebo skid steer plugin
int MicroGateway::ConnectROS(char * addr) { 

  if(this->connectedROS) { 
    PRINT_INFO("Already initialized ROS stuff\n"); 
    return 0; 
  }

  //ROS stuff to initialize
  this->wheelSeparation = 0.39370; 
  this->wheelDiameter = 0.25400; 
  this->torque = 2.5; 
  ...
  //initialize ROS variables and set up subscribers and publishers
  //ros::SubscribeOptions so = ros::SubscribeOptions::create<geometry_msgs::Twist>("/cmd_vel", 1, 
  //										boost::bind(&MicroGateway::cmdVelCallback, this, _1), 
  //										ros::VoidPtr(), &queue_); 
...
/*
void MicroGateway::UpdateChild() { //handles odometry calculation and update

}

void MicroGateway::FiniChild() { 
	this->alive_ = false; 
	this->queue_ .clear(); 
	this->queue_.disable(); 
	this->nh.shutdown(); 
...
/*
void MicroGateway::GetPositionCmd() { 
	//this->lock.lock(); 
...
/*
void MicroGateway::QueueThread() { 
	static const double timeout = 0.01; 

	while(this->alive_ && this->nh.ok()) { 
...
void MicroGateway::ROSCalcOdom(EncoderCounts encPacket) { 
	...
	int16_t fr = encPacket.fr; 
	int16_t fl = encPacket.fl; 
	int16_t rr = encPacket.rr; 
	int16_t rl = encPacket.rl; 
	...
	tf::quaternionTFToMsg(temp_quat, quat); 
	pose.orientation.x = quat.x; 
	pose.orientation.y = quat.y; 
	pose.orientation.z = quat.z; 
	pose.orientation.w = quat.w; 

	btQuaternion qt(pose.orientation.x, pose.orientation.y, pose.orientation.z, pose.orientation.w);  
	btVector3 vt(pose.position.x, pose.position.y, pose.position.z); 
	//tf::Quaternion qt(temp_quat); 
	//tf::Vector vt(pose.position.x, pose.position.y, pose.position.z); 

	tf::Transform base_footprint_to_odom(qt, vt); 
	this->odom_broadcaster.sendTransform(tf::StampedTransform(base_footprint_to_odom, current_time, odom_frame, base_footprint_frame)); 

	//publish odometry topic
	this->odom_.pose.pose.position.x = pose.position.x; 
	this->odom_.pose.pose.position.y = pose.position.y; 
	this->odom_.pose.pose.position.z = pose.position.z; 

	this->odom_.pose.pose.orientation.x = quat.x; 
	this->odom_.pose.pose.orientation.y = quat.y; 
