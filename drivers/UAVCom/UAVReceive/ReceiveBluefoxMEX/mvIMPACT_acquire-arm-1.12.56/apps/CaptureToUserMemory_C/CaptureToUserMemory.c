		...
		if( result == DMR_NO_ERROR )
		{
			...
			if( ( result == DMR_NO_ERROR ) && ( ReqRes.result == rrOK ) )
			{
				...
				if( ( result = DMR_GetImageRequestBuffer( hDrv, requestNr, &pIB ) ) == DMR_NO_ERROR )
				{
					...
					mvDispSetImage( pCaptureParameter->pDisp, pIB->vpData, pIB->iWidth, pIB->iHeight, pIB->pChannels[0].iPixelPitch*8, pIB->pChannels[0].iLinePitch );
...
//-----------------------------------------------------------------------------
{
	...
	captureParams.hDisp = mvInitDisplayWindow( "CaptureToUserMemory sample(plain 'C')" );
	captureParams.pDisp = mvGetDisplayStructure( captureParams.hDisp );
	mvShowDisplayWindow( captureParams.hDisp );
#endif // #ifdef USE_MV_DISPLAY_LIB
...
#ifdef USE_MV_DISPLAY_LIB
				// display the captured image
				mvDispSetImage( captureParams.pDisp, pIB->vpData, pIB->iWidth, pIB->iHeight, pIB->pChannels[0].iPixelPitch*8, pIB->pChannels[0].iLinePitch );
...
#ifdef USE_MV_DISPLAY_LIB
	mvDestroyImageWindow( captureParams.hDisp );
#endif // #ifdef USE_MV_DISPLAY_LIB
	freeRequests( &captureParams );
	freeCaptureBuffers( &captureParams );
