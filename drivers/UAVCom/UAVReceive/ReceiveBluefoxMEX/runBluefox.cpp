...
int compress_bw(uint8_t *prRGB, int width, int height);
...
//-----------------------------------------------------------------------------
{
	...
	while( !pThreadParameter->terminated() )
	{
        ...
        int height = pRequest->imageHeight.read();
        uint8_t * data = (uint8_t *)pRequest->imageData.read();
        printf("got image %dx%d\n : %d bytes", width , height, width*height);
        compress_bw(data,width,height);
        //IPC_publish("image",width*height,pRequest->imageData.read());
        IPC_publish("image",destBuf.size(),&(destBuf[0]));
				...
				if( cnt%100 == 0 )
				{
					cout << "Info from " << pThreadParameter->device()->serial.read() 
						...
						<< statistics.errorCount << ", " << statistics.captureTime_s 
						<< ", " << statistics.frameCount << endl;
				}
			}
			else
			{
...
//-----------------------------------------------------------------------------
{
  ...
  IPC_defineMsg("image",IPC_VARIABLE_LENGTH,NULL);
