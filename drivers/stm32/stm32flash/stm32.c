...
char stm32_erase_memory(const stm32_t *stm) {
	...
	if (!stm32_send_command(stm, 0xFF        )) return 0;  //0xFF for mass erase
	return 1;
}

char stm32_erase_pages(const stm32_t *stm, uint8_t size) {
  int ii;
  uint8_t cs;
  if (!stm32_send_command(stm, stm->cmd->er)) return 0;
  ...
  if (stm32_read_byte(stm) != STM32_ACK) {
    fprintf(stderr, "Error sending erase cmd to device\n");
    return 0;
  }
