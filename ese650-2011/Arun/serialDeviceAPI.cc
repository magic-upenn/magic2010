...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]){
	...
	if (strcasecmp(command, "connect") == 0) {
		...
		if (pDev->Connect(deviceName,baud) || pDev->Set_IO_BLOCK_W_TIMEOUT()){
			delete pDev;
			pDev=NULL;
      mexErrMsgTxt("serialDeviceAPI: Could not open device");
			//std::cout << "serialDeviceAPI: Could not open device" << std::endl;
  ...
  else if (strcasecmp(command, "setTermSequenceEndLine") == 0) {
    pDev->Set_IO_BLOCK_W_TIMEOUT_W_TERM_SEQUENCE("\r\n",2,true);
    plhs[0] = mxCreateDoubleScalar(1);
    return;
  }
      ...
      if (numRead > 0)
		...
		if (mxGetString(prhs[1], data, BUFLEN) != 0) {
			mexErrMsgTxt("serialDeviceAPI: Could not read string.");
		}

		int len=strlen(data);

		if (pDev->WriteChars(data,len) == len){
