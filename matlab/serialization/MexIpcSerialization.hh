...
#define MEX_IPC_SERIALIZATION
  ...
  #define MEX_READ_STRING( mxArr, index, fieldName, cntr ) \
  { \
    mxArray * mxArrTemp = mxGetField(mxArr,index,#fieldName); \
    if (mxArrTemp) \
    { \
      if (mxGetNumberOfElements(mxArrTemp) == 0) \
        mexErrMsgTxt("field "#fieldName" is empty"); \
      if (mxGetClassID(mxArrTemp) != mxCHAR_CLASS) \
        mexErrMsgTxt("field "#fieldName" is not char"); \
      fieldName = (char*)mxGetData(mxArrTemp); \
      cntr++; \
    } \
  }
  
  #define MEX_READ_FIELD_CUSTOM( mxArr, index, fieldName, cntr ) \
  { \
    mxArray * mxArrTemp = mxGetField(mxArr,index,#fieldName); \
    if (mxArrTemp) \
    { \
      if (fieldName.ReadFromMatlab(mxArrTemp,0) < 1) \
        mexErrMsgTxt("could not field "#fieldName); \
      cntr++; \
    } \
  }
  ...
  #define MEX_WRITE_FIELD_CUSTOM(mxArr,index,fieldName) \
  {\
    mxArray * mxCustom; \
    fieldName.CreateMatlabStructMatrix(&mxCustom,1,1); \
    fieldName.WriteToMatlab(mxCustom,0); \
    mxSetField(mxArr,index,#fieldName,mxCustom); \
  }
  
  #define MEX_WRITE_STRING(mxArr,index,fieldName) \
  { \
    mxSetField(mxArr,index,#fieldName,mxCreateString(fieldName)); \
  }
